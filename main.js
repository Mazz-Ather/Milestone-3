...
let registeredUsers = []; // Array to store user information (name, email, number, password)
async function main() {
    while (true) {
...
async function handleBrowseEvents() {
    ...
    if (registeredUsers <= 0) {
        ...
        if (events.choice === " Flight Ticket") {
            ...
            async function selectOriginAndDestination(fromCityChoices, toCityChoices) {
...
async function handleRegistration() {
    ...
    if (registeredUsers.some((user) => user.name === newUser.name || user.email === newUser.email)) {
    ...
    console.log(chalk.yellow("*****************************************************************************"));
    console.log(chalk.green(`Welcome! ${newUser.name} You are now registered with our one-stop ticketing platform.`));
    console.log(chalk.green("Get ready to book tickets for all your favorite Countries:"));
    console.log(chalk.yellow("*****************************************************************************"));
    console.log("***************************");
...
async function handleSignIn() {
    ...
    if (signIn.choices === "User") {
        ...
        const foundUser = registeredUsers.find((user) => user.name === login.name); // Find user by name in registeredUsers
        ...
        },
        {
            ...
            validate: function (value) {
                if (isNaN(value) || value < 0) {
                    return chalk.red("Please Enter A Valid Ticket Stock (positive number).");
                }
                return true;
            },
        },
    ]);
    console.log(`you make ${newEvent.title} event , which will be held on ${newEvent.time} in ${newEvent.city} city, the total stock of ticket is ${newEvent.stock}`);
}
//?===================================================================================================================
//*                              view users
//!_----------------------------------------------------------------------------------------------------------------
        ...
        if (charges.charges === "paypal") {
            ...
            const login = await inquirer.prompt([
                ...
                },
                {
                    type: "number",
                    name: "number",
                    message: chalk.grey("Enter Your Paypal Account Number (must be 11 digits):"),
                    validate: function (value) {
                        if (isNaN(value) || value.toString().length !== 11) {
                            return chalk.red("Please enter a valid account number (11 digits).");
                        }
                        return true;
                    },
                },
            ...
            const login = await inquirer.prompt([
                ...
                },
                {
                    type: "number",
                    name: "credit",
                    message: chalk.grey("Enter Your credit card Account Number (must be 11 digits):"),
                    validate: function (value) {
                        if (isNaN(value) || value.toString().length !== 11) {
                            return chalk.red("Please enter a valid account number (11 digits).");
                        }
                ...
                if (method.method === "Quit") {
                    ...
                    if (myBalance >= totalCharge) {
                        ...
                        if (paymentConfirmation.pay) {
                            ...
                            console.log(chalk.greenBright("\nTickets booked successfully!"));
                            console.log("*************************************************");
                            console.log(`Your remaining Balance is ${myBalance - totalCharge}`);
                        }
                        else {
                            console.log("----------------------------------------------------------");
                            console.log(chalk.yellow("\nPayment not confirmed. You have to pay first to book tickets."));
